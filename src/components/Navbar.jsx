...
// ============ Animation Keyframes ============
const starBtnMove = keyframes`
  0% { offset-distance: 0%; }
  100% { offset-distance: 100%; }
`;

// ============ Styled StarButton ============
const StyledStarButton = styled(Button)(({ theme }) => ({
  position: "relative",
  zIndex: 3,
  overflow: "hidden",
  height: "40px",
  ...
  "& .star-light": {
    ...
    aspectRatio: "1",
    inset: 0,
    width: "var(--light-width, 110px)",
    background:
      "radial-gradient(ellipse at center, var(--light-color, #FAFAFA), transparent, transparent)",
    animation: `${starBtnMove} var(--duration, 3s) linear infinite`,
    offsetPath: "var(--path)",
    offsetDistance: "0%",
    zIndex: 1,
  },

  // Border with StarBackground
  "& .star-border": {
    position: "absolute",
    inset: 0,
    ...
    border:
      theme.palette.mode === "dark"
        ? "var(--border-width, 2px) solid rgba(255,255,255,0.15)"
        : "var(--border-width, 2px) solid rgba(0,0,0,0.1)",
  ...
  "& .star-text": {
    ...
    background: `linear-gradient(to top, ${
      theme.palette.mode === "dark" ? "#fff, #888" : "#000, #666"
    })`,
...
function StarButtonMUI({
  ...
  useEffect(() => {
    if (pathRef.current) {
      ...
      div.style.setProperty("--path", `path('M 0 0 H ${w} V ${h} H 0 V 0')`);
      div.style.setProperty("--light-width", `${lightWidth}px`);
      div.style.setProperty("--duration", `${duration}s`);
      div.style.setProperty("--light-color", lightColor);
      div.style.setProperty("--border-width", `${borderWidth}px`);
    }
  }, [lightWidth, duration, lightColor, borderWidth]);

  return (
    <StyledStarButton ref={pathRef} onClick={onClick} sx={sx} {...props}>
      <span className="star-light" />
      <span className="star-border" aria-hidden="true">
        <StarBackground color={backgroundColor} />
      </span>
      <span className="star-text">{children}</span>
    </StyledStarButton>
  );
}
        ...
        return (
          ...
          >
            <StarButtonMUI
              ...
              sx={mobile ? { width: "100%", height: "48px" } : {}}
            >
              {item.label}
            </StarButtonMUI>
          </motion.div>
    ...
    >
      ...
      <MotionAppBar
        ...
        sx={{
          ...
          borderRadius: {
            xs: "24px",     // ← Mobile: أقل استدارة
            sm: "40px",
            md: "80px",     // ← Desktop: زي ما كان
          },
          minHeight: {
            xs: "52px",
            sm: "58px",
          ...
          {isMobile && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              <StarButtonMUI
                ...
                sx={{ height: "36px", padding: "0 16px", fontSize: "0.8rem" }}
                lightWidth={80}
              >
                Contact
              </StarButtonMUI>
